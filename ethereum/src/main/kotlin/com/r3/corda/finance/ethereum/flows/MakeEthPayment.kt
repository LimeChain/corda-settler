...
import com.r3.corda.finance.ethereum.utilities.DEFAULT_ETH_FEE
import com.r3.corda.finance.ethereum.utilities.ETH
import com.r3.corda.finance.ethereum.utilities.toETHAmount
import com.r3.corda.finance.ethereum.utilities.toETHHash
...
import com.ripple.core.coretypes.Amount as EthereumAmount
...
class MakeEthPayment<T : Money>(
    ...
    private fun getSequenceNumber(): UInt32 {
        val ethService = serviceHub.cordaService(ETHService::class.java).client
        val accountId = ethService.address
        return ethService.nextSequenceNumber(accountId)
    }

    /** Don't want to serialize this either. */
    private fun getCurrentLedgerIndex(): Long {
        val ethService = serviceHub.cordaService(ETHService::class.java).client
        return ethService.ledgerIndex().ledgerCurrentIndex
    }
    ...
    private fun createAndSignAndSubmitPayment(obligation: Obligation<*>, amount: Amount<T>): SubmitPaymentResponse {
        ...
        // So if this flow is restarted and the payment were to be made twice then the Ethereum node will return an error
        // because the same sequence number will be used twice.
        val payment = ethService.createPayment(
                // Always use the sequence number provided. It will never be null at this point.
                sequence = seqNo!!,
                source = ethService.address,
                destination = (settlementMethod as EthSettlement).accountToPay,
                amount = amount.toETHAmount(),
                fee = DEFAULT_ETH_FEE,
                linearId = SecureHash.sha256(obligation.linearId.id.toString()).toETHHash()
        )

        // 2. Sign the payment.
        val signedPayment = ethService.signPayment(payment)
        return ethService.submitTransaction(signedPayment)
    }
    ...
    override fun checkBalance(requiredAmount: Amount<*>) {
        // Get a ETHService client.
        val ethClient = serviceHub.cordaService(ETHService::class.java).client
        // Check the balance on the supplied ETHService address.
        val ourAccountInfo = ethClient.accountInfo(ethClient.address)
        val balance = ourAccountInfo.accountData.balance
        // ETH accounts must contain a minimum of 20 ETH.
        check(balance > requiredAmount.toETHAmount().add(20)) {
    ...
    override fun makePayment(obligation: Obligation<*>, amount: Amount<T>): EthPayment<T> {
        ...
        val paymentResponse = try {
            ...
            throw FlowException("The transaction was already submitted. However, " +
                    ...
                    "Please check your ETH payments to obtain the transaction hash " +
            ...
            throw FlowException("An incorrect sequence number was used. This could be " +
                    "due to a race with another application to submit a ethereum transaction." +
        ...
        return EthPayment(paymentReference, lastLedgerIndex, amount, PaymentStatus.SENT)
    }
}