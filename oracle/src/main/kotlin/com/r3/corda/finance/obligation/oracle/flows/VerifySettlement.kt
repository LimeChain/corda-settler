...
import com.r3.corda.finance.obligation.oracle.services.EthOracleService
...
import com.r3.corda.finance.ethereum.types.EthSettlement
...
class VerifySettlement(val otherSession: FlowSession) : FlowLogic<Unit>() {
    ...
    @Suspendable
    fun verifyEthSettlement(obligation: Obligation<DigitalCurrency>, ethPayment: EthPayment<DigitalCurrency>): VerifyResult {
        val oracleService = serviceHub.cordaService(EthOracleService::class.java)
        while (true) {
            logger.info("Checking for settlement...")
            val result = oracleService.hasPaymentSettled(ethPayment, obligation)
            when (result) {
                VerifyResult.SUCCESS, VerifyResult.TIMEOUT -> return result
                // Sleep for five seconds before we try again. The Oracle might receive the request to verify payment
                // before the payment succeed. Also it takes a bit of time for all the nodes to receive the new ledger
    ...
    override fun call() {
        ...
        val verifyResult = when (settlementMethod) {
            ...
            is EthSettlement -> verifyEthSettlement(obligation as Obligation<DigitalCurrency>, lastPayment as EthPayment<DigitalCurrency>)
